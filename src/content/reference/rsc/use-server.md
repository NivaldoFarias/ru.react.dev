...
При вызове Server Function на клиенте будет выполнен сетевой запрос на сервер, включающий сериализованную копию всех переданных аргументов. Если Server Function возвращает значение, это значение будет сериализовано и возвращено клиенту.
...
#### Ограничения {/*caveats*/}
* `'use server'` должна быть в самом начале функции или модуля; выше любого другого кода, включая импорты (комментарии перед директивами допустимы). Она должна быть написана с одинарными или двойными кавычками, а не с обратными.
...
* Чтобы импортировать Server Functions из [клиентского кода](/reference/rsc/use-client), директива должна использоваться на уровне модуля.
* Поскольку базовые сетевые вызовы всегда асинхронны, `'use server'` может использоваться только с асинхронными функциями.
* Всегда относитесь к аргументам Server Functions как к недоверенному вводу и авторизуйте любые мутации. См. [соображения безопасности](#security).
...
* Server Functions предназначены для мутаций, обновляющих серверное состояние; они не рекомендуются для получения данных. Соответственно, фреймворки, реализующие Server Functions, обычно обрабатывают одно действие за раз и не имеют способа кешировать возвращаемое значение.

### Соображения безопасности {/*security*/}

Аргументы для Server Functions полностью контролируются клиентом. В целях безопасности всегда относитесь к ним как к недоверенному вводу и убедитесь, что аргументы валидированы и экранированы соответствующим образом.

В любой Server Function убедитесь, что вы проверяете, разрешено ли вошедшему в систему пользователю выполнять это действие.

<Wip>

Чтобы предотвратить отправку конфиденциальных данных из Server Function, существуют экспериментальные API для "загрязнения" (taint), которые предотвращают передачу уникальных значений и объектов в клиентский код.
...
Важно отметить, что не поддерживаются:
...
Наиболее распространенным сценарием использования Server Functions будет вызов функций, которые изменяют данные. В браузере [HTML-элемент form](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form) является традиционным подходом для пользователя к отправке мутаций. С React Server Components React представляет первоклассную поддержку Server Functions в качестве Actions в [формах](/reference/react-dom/components/form).
...
В этом примере `requestUsername` — это Server Function, переданная в `<form>`. Когда пользователь отправляет эту форму, выполняется сетевой запрос к серверной функции `requestUsername`. При вызове Server Function в форме React передаст [FormData](https://developer.mozilla.org/en-US/docs/Web/API/FormData) формы в качестве первого аргумента Server Function.
...
В форме запроса имени пользователя может возникнуть ситуация, когда имя пользователя недоступно. `requestUsername` должен сообщить нам, удалась ли операция или нет.

Чтобы обновить UI на основе результата Server Function при поддержке постепенного улучшения, используйте [`useActionState`](/reference/react/useActionState).
...
Server Functions являются серверными конечными точками и могут быть вызваны из любого места в клиентском коде.
