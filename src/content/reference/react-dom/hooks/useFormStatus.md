...
Чтобы получить информацию о статусе, компонент `Submit` должен быть отрендерен внутри `<form>`. Хук возвращает информацию, такую как свойство <CodeStep step={1}>`pending`</CodeStep>, которое указывает, происходит ли в данный момент отправка формы.
...
* `method`: Строковое значение `'get'` или `'post'`. Оно указывает, отправляет ли родительская форма `<form>` с использованием метода [HTTP](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods) `GET` или `POST`. По умолчанию `<form>` использует метод `GET`, и это можно указать с помощью свойства [`method`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form#method).
...
* Хук `useFormStatus` должен быть вызван из компонента, который рендерится внутри `<form>`.
* `useFormStatus` будет возвращать информацию о статусе только для родительской формы `<form>`. Он не будет возвращать информацию о статусе для любой формы `<form>`, отрендеренной в том же компоненте или в дочерних компонентах.
...
Чтобы отобразить состояние ожидания во время отправки формы, вы можете вызвать хук `useFormStatus` в компоненте, отрендеренном внутри `<form>`, и прочитать возвращаемое свойство `pending`.
...
##### `useFormStatus` не вернет информацию о статусе для `<form>`, отрендеренной в том же компоненте. {/*useformstatus-will-not-return-status-information-for-a-form-rendered-in-the-same-component*/}

Хук `useFormStatus` возвращает информацию о статусе только для родительской формы `<form>`, а не для любой формы `<form>`, отрендеренной в том же компоненте, который вызывает хук, или в дочерних компонентах.

```js
function Form() {
  // 🚩 `pending` никогда не будет true
  // useFormStatus не отслеживает форму, отрендеренную в этом компоненте
  const { pending } = useFormStatus();
  return <form action={submit}></form>;
}
```

Вместо этого вызовите `useFormStatus` изнутри компонента, который находится внутри `<form>`.
...
Если компонент, вызывающий `useFormStatus`, не вложен в `<form>`, `status.pending` всегда будет возвращать `false`. Убедитесь, что `useFormStatus` вызывается в компоненте, который является дочерним элементом элемента `<form>`.

`useFormStatus` не будет отслеживать статус формы `<form>`, отрендеренной в том же компоненте. См. [Pitfall](#useformstatus-will-not-return-status-information-for-a-form-rendered-in-the-same-component) для получения более подробной информации.