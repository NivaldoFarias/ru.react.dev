...
`prefetchDNS` позволяет вам заранее выполнить поиск IP-адреса сервера, с которого вы ожидаете загрузить ресурсы.
...
Функция `prefetchDNS` предоставляет браузеру подсказку о том, что ему следует выполнить поиск IP-адреса указанного сервера. Если браузер решит это сделать, это может ускорить загрузку ресурсов с этого сервера.
