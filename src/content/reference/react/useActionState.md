...
{/* TODO T164397693: добавить ссылку на документацию по действиям, как только она появится */}

Вызовите `useActionState` на верхнем уровне вашего компонента, чтобы создать состояние компонента, которое обновляется [при вызове действия формы](/reference/react-dom/components/form). Вы передаете `useActionState` существующую функцию действия формы, а также начальное состояние, и он возвращает новое действие, которое вы используете в своей форме, вместе с последним состоянием формы и индикатором того, выполняется ли действие в данный момент. Последнее состояние формы также передается в функцию, которую вы предоставили.
...
* `initialState`: Значение, которое должно быть у состояния изначально. Это может быть любое сериализуемое значение. Этот аргумент игнорируется после первого вызова действия.
* **необязательный** `permalink`: Строка, содержащая уникальный URL страницы, которую изменяет эта форма. Используется на страницах с динамическим контентом (например, лентами) в сочетании с прогрессивным улучшением: если `fn` является [серверной функцией](/reference/rsc/server-functions), и форма отправляется до загрузки JavaScript-пакета, браузер перейдет по указанному URL `permalink`, а не по URL текущей страницы. Убедитесь, что тот же компонент формы отрисован на целевой странице (включая то же действие `fn` и `permalink`), чтобы React знал, как передать состояние. После гидратации формы этот параметр не оказывает никакого влияния.

{/* TODO T164397693: добавить ссылку на документацию по сериализуемым значениям, как только она появится */}
...
2. Новое действие, которое вы можете передать в качестве `action` пропса вашему `form` компоненту или `formAction` пропса любому `button` компоненту внутри формы. Действие также можно вызвать вручную в [`startTransition`](/reference/react/startTransition).
3. Флаг `isPending`, который сообщает, есть ли ожидающий переход (Transition).
...
* Функция, передаваемая в `useActionState`, получает дополнительный аргумент — предыдущее или начальное состояние — в качестве первого аргумента. Это делает ее сигнатуру отличной от той, если бы она использовалась напрямую как действие формы без использования `useActionState`.
...
2. <CodeStep step={2}>новое действие</CodeStep>, которое вы передаете в `<form>` как его `action` пропс или вызываете вручную внутри `startTransition`.
3. <CodeStep step={1}>состояние ожидания</CodeStep> (`pending state`), которое вы можете использовать, пока ваше действие обрабатывается.
...
Чтобы отобразить сообщения, такие как сообщение об ошибке или уведомление (toast), возвращаемое серверной функцией, оберните действие в вызов `useActionState`.
...
Когда вы оборачиваете действие с помощью `useActionState`, оно получает дополнительный аргумент *в качестве первого аргумента*. Поэтому отправленные данные формы становятся его *вторым* аргументом, а не первым, как это обычно бывает. Новый первый аргумент, который добавляется, — это текущее состояние формы.
