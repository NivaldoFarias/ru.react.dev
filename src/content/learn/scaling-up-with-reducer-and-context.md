...
В этом примере из [введения в редюсеры](/learn/extracting-state-logic-into-a-reducer) состояние управляется редюсером. Функция редюсера содержит всю логику обновления состояния и объявлена внизу этого файла:
...
Редюсер помогает сделать обработчики событий короткими и лаконичными. Однако по мере роста вашего приложения вы можете столкнуться с другой трудностью. **В настоящее время состояние `tasks` и функция `dispatch` доступны только в компоненте верхнего уровня `TaskApp`.** Чтобы другие компоненты могли читать список задач или изменять его, вам придется явно [передавать](/learn/passing-props-to-a-component) текущее состояние и обработчики событий, которые его изменяют, в качестве пропсов.
...
В таком небольшом примере это работает хорошо, но если у вас есть десятки или сотни компонентов посередине, передача всех состояний и функций через пропсы может быть довольно утомительной!

Именно поэтому, в качестве альтернативы передаче через пропсы, вы можете захотеть поместить как состояние `tasks`, так и функцию `dispatch` [в контекст.](/learn/passing-data-deeply-with-context) **Таким образом, любой компонент ниже `TaskApp` в дереве сможет читать задачи и отправлять действия без повторяющейся «проп-дрелинг» (prop drilling).**
...
Экспортируйте их из отдельного файла, чтобы вы могли импортировать их из других файлов позже:
...
Здесь вы передаете `null` в качестве значения по умолчанию обоим контекстам. Фактические значения будут предоставлены компонентом `TaskApp`.

### Шаг 2: Поместите состояние и `dispatch` в контекст {/*step-2-put-state-and-dispatch-into-context*/}

Теперь вы можете импортировать оба контекста в ваш компонент `TaskApp`. Возьмите `tasks` и `dispatch`, возвращаемые `useReducer()`, и [предоставьте их](/learn/passing-data-deeply-with-context#step-3-provide-the-context) всему дереву ниже:
...
В следующем шаге вы удалите передачу пропсов.
...
**Компонент `TaskApp` не передает никаких обработчиков событий вниз, и `TaskList` также не передает никаких обработчиков событий компоненту `Task`.** Каждый компонент читает тот контекст, который ему нужен:
...
Этот файл скоро станет переполненным! Вы переместите редьюсер в тот же файл. Затем вы объявите новый компонент `TasksProvider` в том же файле. Этот компонент объединит все части:
...
Вы можете думать о `TasksProvider` как о части экрана, которая знает, как работать с задачами, `useTasks` — как о способе их чтения, а `useTasksDispatch` — как о способе их обновления из любого компонента ниже в дереве.
...
- Вы можете объединить редьюсер с контекстом, чтобы любой компонент мог читать и обновлять состояние над ним.
- Чтобы предоставить состояние и функцию `dispatch` компонентам ниже:
  ...
  3. Используйте любой из контекстов из компонентов, которым нужно их прочитать.
