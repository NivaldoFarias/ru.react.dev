---
title: "Руководство по обновлению до React 19"
author: Ricky Hanlon
date: 2024/04/25
description: Улучшения, добавленные в React 19, требуют некоторых критических изменений, но мы постарались сделать обновление максимально плавным, и не ожидаем, что изменения повлияют на большинство приложений. В этой статье мы проведем вас через шаги по обновлению приложений и библиотек до React 19.
---

25 апреля 2024 г. [Ricky Hanlon](https://twitter.com/rickhanlonii)
...
Улучшения, добавленные в React 19, требуют некоторых критических изменений, но мы постарались сделать обновление максимально плавным, и не ожидаем, что изменения повлияют на большинство приложений.

</Intro>

<Note>

#### Также выпущен React 18.3 {/*react-18-3*/}

Чтобы облегчить обновление до React 19, мы выпустили релиз `react@18.3`, который идентичен 18.2, но добавляет предупреждения об устаревших API и других изменениях, необходимых для React 19.

Мы рекомендуем сначала обновиться до React 18.3, чтобы выявить возможные проблемы перед обновлением до React 19.

Список изменений в 18.3 см. в [Заметках к выпуску](https://github.com/facebook/react/blob/main/CHANGELOG.md#1830-april-25-2024).
...
- [Новые устаревшие API](#new-deprecations)
- [Заметные изменения](#notable-changes)
- [Изменения TypeScript](#typescript-changes)
- [Changelog](#changelog)

Если вы хотите помочь нам протестировать React 19, следуйте инструкциям в этом руководстве по обновлению и [сообщайте о любых проблемах](https://github.com/facebook/react/issues/new?assignees=&labels=React+19&projects=&template=19.md&title=%5BReact+19%5D), с которыми вы столкнетесь. Список новых функций, добавленных в React 19, см. в [посте о выпуске React 19](/blog/2024/12/05/react-19).
...
Изменения, включающие codemod, указаны ниже с соответствующей командой.
...
### Ошибки при рендеринге больше не перевыбрасываются {/*errors-in-render-are-not-re-thrown*/}

В предыдущих версиях React ошибки, возникающие во время рендеринга, перехватывались и перевыбрасывались. В режиме разработки (DEV) они также выводились в `console.error`, что приводило к дублированию логов ошибок.

В React 19 мы [улучшили обработку ошибок](/blog/2024/04/25/react-19#error-handling), чтобы уменьшить дублирование, не перевыбрасывая ошибки:

*   **Неперехваченные ошибки**: Ошибки, которые не были перехвачены Error Boundary, сообщаются через `window.reportError`.
*   **Перехваченные ошибки**: Ошибки, которые были перехвачены Error Boundary, сообщаются через `console.error`.

Это изменение не должно повлиять на большинство приложений, но если ваша система отчётности об ошибках в продакшене полагается на перевыбрасывание ошибок, вам может потребоваться обновить её. Для поддержки этого мы добавили новые методы в `createRoot` и `hydrateRoot` для пользовательской обработки ошибок:
...
#### Удалены: `propTypes` и `defaultProps` для функций {/*removed-proptypes-and-defaultprops*/}
...
В React 19 мы удаляем проверки `propTypes` из пакета React, и их использование будет тихо игнорироваться. Если вы используете `propTypes`, мы рекомендуем перейти на TypeScript или другое решение для проверки типов.
...
Преобразуйте `propTypes` в TypeScript с помощью:
...
Устаревший контекст был объявлен устаревшим в [октябре 2018 года (v16.6.0)](https://legacy.reactjs.org/blog/2018/10/23/react-v-16-6.html).

Устаревший контекст был доступен только в классовых компонентах с использованием API `contextTypes` и `getChildContext` и был заменён на `contextType` из-за тонких ошибок, которые было легко пропустить. В React 19 мы удаляем устаревший контекст, чтобы сделать React немного меньше и быстрее.

Если вы всё ещё используете устаревший контекст в классовых компонентах, вам нужно будет перейти на новый API `contextType`:
...
#### Удалены: строковые рефы {/*removed-string-refs*/}
Строковые рефы были объявлены устаревшими в [марте 2018 года (v16.3.0)](https://legacy.reactjs.org/blog/2018/03/27/update-on-async-rendering.html).
...
Если вы всё ещё используете строковые рефы в классовых компонентах, вам нужно будет перейти на колбэки рефов:
...
Преобразуйте строковые рефы в колбэки рефов с помощью codemod:
...
#### Удалены: фабрики шаблонных модулей {/*removed-module-pattern-factories*/}
Фабрики шаблонных модулей были объявлены устаревшими в [августе 2019 года (v16.9.0)](https://legacy.reactjs.org/blog/2019/08/08/react-v16.9.0.html#deprecating-module-pattern-factories).

Этот шаблон использовался редко, и его поддержка делает React немного больше и медленнее, чем необходимо. В React 19 мы удаляем поддержку фабрик шаблонных модулей, и вам нужно будет перейти на обычные функции:
...
#### Удален: `React.createFactory` {/*removed-createfactory*/}
...
Использование `createFactory` было обычным делом до широкой поддержки JSX, но сегодня оно используется редко и может быть заменено на JSX. В React 19 мы удаляем `createFactory`, и вам нужно будет перейти на JSX:
...
#### Удален: `react-test-renderer/shallow` {/*removed-react-test-renderer-shallow*/}
...
Поверхностное рендерирование зависит от внутренних механизмов React и может помешать вам при будущих обновлениях. Мы рекомендуем перенести ваши тесты на [@testing-library/react](https://testing-library.com/docs/react-testing-library/intro/) или [@testing-library/react-native](https://testing-library.com/docs/react-native-testing-library/intro).

</Note>

### Удалены устаревшие API React DOM {/*removed-deprecated-react-dom-apis*/}

#### Удален: `react-dom/test-utils` {/*removed-react-dom-test-utils*/}
...
`ReactDOMTestUtils.act` устарел в пользу `React.act`. Импортируйте `act` из `react`, а не из `react-dom/test-utils`. Подробнее см. https://react.dev/warnings/react-dom-test-utils.
...
Все остальные функции `test-utils` были удалены. Эти утилиты использовались редко, и они слишком упрощали зависимость от низкоуровневых деталей реализации ваших компонентов и React. В React 19 вызов этих функций приведёт к ошибке, а их экспорт будет удалён в будущей версии.

См. [страницу предупреждений](https://react.dev/warnings/react-dom-test-utils) для альтернатив.

<Note>

Преобразуйте `ReactDOMTestUtils.act` в `React.act` с помощью codemod:
...
#### Удален: `ReactDOM.render` {/*removed-reactdom-render*/}
...
Преобразуйте `ReactDOM.render` в `ReactDOMClient.createRoot` с помощью codemod:
...
#### Удален: `ReactDOM.hydrate` {/*removed-reactdom-hydrate*/}
...
Преобразуйте `ReactDOM.hydrate` в `ReactDOMClient.hydrateRoot` с помощью codemod:
...
#### Удален: `unmountComponentAtNode` {/*removed-unmountcomponentatnode*/}
...
Преобразуйте `unmountComponentAtNode` в `root.unmount` с помощью codemod:
...
#### Удален: `ReactDOM.findDOMNode` {/*removed-reactdom-finddomnode*/}
...
Мы удаляем `findDOMNode`, потому что это был устаревший обходной путь, который медленно выполнялся, был хрупким к рефакторингу, возвращал только первого потомка и нарушал уровни абстракции (подробнее см. [здесь](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Вы можете заменить `ReactDOM.findDOMNode` на [DOM-рефы](/learn/manipulating-the-dom-with-refs):
...
React 19 поддерживает [`ref` как проп](/blog/2024/04/25/react-19#ref-as-a-prop), поэтому мы делаем `element.ref` устаревшим в пользу `element.props.ref`.
...
Мы делаем `react-test-renderer` устаревшим, поскольку он реализует собственную среду рендеринга, которая не соответствует среде, используемой пользователями, способствует тестированию деталей реализации и полагается на интроспекцию внутренних механизмов React.

Тестовый рендерер был создан до появления более жизнеспособных стратегий тестирования, таких как [React Testing Library](https://testing-library.com), и теперь мы рекомендуем использовать современную библиотеку для тестирования.
...
## Важные изменения {/*notable-changes*/}
...
Как и все поведение Strict Mode, эти функции предназначены для проактивного выявления ошибок в ваших компонентах во время разработки, чтобы вы могли исправить их до выпуска в продакшен. Например, во время разработки Strict Mode будет дважды вызывать функции обратного вызова ref при первоначальном монтировании, чтобы имитировать то, что происходит, когда смонтированный компонент заменяется резервным вариантом Suspense.

### Улучшения Suspense {/*improvements-to-suspense*/}

В React 19, когда компонент приостанавливается (suspends), React немедленно применит резервный вариант ближайшей границы Suspense, не дожидаясь рендеринга всего дерева братьев и сестер. После применения резервного варианта React планирует еще один рендеринг для приостановленных братьев и сестер, чтобы "предварительно разогреть" ленивые запросы в остальной части дерева:

<Diagram name="prerender" height={162} width={1270} alt="Диаграмма, показывающая дерево из трех компонентов, один родительский компонент с именем Accordion и два дочерних компонента с именем Panel. Оба компонента Panel содержат isActive со значением false.">

Ранее, когда компонент приостанавливался, рендерились приостановленные братья и сестры, а затем применялся резервный вариант.
...
В React 19, когда компонент приостанавливается, применяется резервный вариант, а затем рендерится приостановленное дерево братьев и сестер.

</Diagram>

Это изменение означает, что резервные варианты Suspense отображаются быстрее, при этом "разогревая" ленивые запросы в приостановленном дереве.
...
Этот выпуск включает изменения во внутренних механизмах React, которые могут повлиять на библиотеки, игнорирующие наши просьбы не использовать внутренние механизмы, такие как `SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED`. Эти изменения необходимы для внедрения улучшений в React 19 и не нарушат работу библиотек, следующих нашим рекомендациям.

На основе нашей [Политики версионирования](https://react.dev/community/versioning-policy#what-counts-as-a-breaking-change), эти обновления не перечислены как ломающие изменения, и мы не предоставляем документацию по их обновлению. Рекомендация — удалить любой код, который зависит от внутренних механизмов.
...
<Note>
Мы опубликовали [`types-react-codemod`](https://github.com/eps1lon/types-react-codemod/) для миграции большинства ломающих изменений, связанных с типами:
...
Из-за введения функций очистки ref, возврат чего-либо, кроме `undefined`, из callback-функции ref теперь будет отклоняться TypeScript. Исправление обычно заключается в прекращении использования неявных возвратов:
...
Исходный код возвращал экземпляр `HTMLDivElement`, и TypeScript не знал, должна ли это быть функция очистки или нет.
...
Давняя жалоба на то, как TypeScript и React работают, — это `useRef`. Мы изменили типы так, что `useRef` теперь требует аргумент. Это значительно упрощает его сигнатуру типа. Теперь он будет вести себя больше как `createContext`.
...
Теперь это также означает, что все ref являются изменяемыми. Вы больше не столкнетесь с проблемой, когда не можете изменить ref, потому что инициализировали его с помощью `null`:
...
`MutableRef` теперь устарел в пользу единого типа `RefObject`, который `useRef` всегда возвращает:
...
Ознакомьтесь с [[RFC] Make all refs mutable](https://github.com/DefinitelyTyped/DefinitelyTyped/pull/64772) для предварительного обсуждения этого изменения.

### Изменения типа `ReactElement` в TypeScript {/*changes-to-the-reactelement-typescript-type*/}
...
`props` React-элементов теперь по умолчанию `unknown`, а не `any`, если элемент типизирован как `ReactElement`. Это не повлияет на вас, если вы передаете аргумент типа в `ReactElement`:
...
Вам это понадобится только в том случае, если у вас много устаревшего кода, полагающегося на некорректный доступ к props элемента. Интроспекция элементов существует только как "лазейка", и вы должны явно указать, что ваш доступ к props некорректен, с помощью явного `any`.
...
Давний запрос — удалить глобальное пространство имен `JSX` из наших типов в пользу `React.JSX`. Это помогает предотвратить загрязнение глобальных типов, что предотвращает конфликты между различными библиотеками пользовательского интерфейса, использующими JSX.

Теперь вам нужно будет обернуть модульную аугментацию пространства имен JSX в `declare module "....":`
...
### Улучшенные типизации `useReducer` {/*better-usereducer-typings*/}

`useReducer` теперь имеет улучшенное выведение типов благодаря [@mfp22](https://github.com/mfp22).

Однако это потребовало ломающего изменения, при котором `useReducer` не принимает полный тип редьюсера в качестве параметра типа, а вместо этого либо не требует никакого (и полагается на контекстное типирование), либо требует как тип состояния, так и тип действия.
...
```
Если вы определяете редьюсер в строке, мы рекомендуем вместо этого аннотировать параметры функции:
```diff
- useReducer<React.Reducer<State, Action>>((state, action) => state)
+ useReducer((state: State, action: Action) => state)
```
Это также то, что вам придется сделать, если вы вынесете редьюсер за пределы вызова `useReducer`:
...
- **react**: Обнаружение бесконечных циклов обновлений, вызванных обновлениями на этапе рендеринга [#26625](https://github.com/facebook/react/pull/26625)
- **react-dom**: Переходы в popstate теперь синхронны [#26025](https://github.com/facebook/react/pull/26025)
- **react-dom**: Удалено предупреждение layout effect во время SSR [#26395](https://github.com/facebook/react/pull/26395)
- **react-dom**: Предупреждать и не устанавливать пустую строку для src/href (кроме тегов якоря) [#28124](https://github.com/facebook/react/pull/28124)
...
Спасибо [Andrew Clark](https://twitter.com/acdlite), [Eli White](https://twitter.com/Eli_White), [Jack Pope](https://github.com/jackpope), [Jan Kassens](https://github.com/kassens), [Josh Story](https://twitter.com/joshcstory), [Matt Carroll](https://twitter.com/mattcarrollcode), [Noah Lemen](https://twitter.com/noahlemen), [Sophie Alpert](https://twitter.com/sophiebits) и [Sebastian Silbermann](https://twitter.com/sebsilbermann) за проверку и редактирование этой публикации.