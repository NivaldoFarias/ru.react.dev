...
description: Улучшения, добавленные в React 19, требуют некоторых критических
  изменений, но мы постарались сделать обновление максимально плавным, и не
  ожидаем, что эти изменения повлияют на большинство приложений. В этой статье
  мы проведем вас через шаги по обновлению приложений и библиотек до React 19.
---
...
Чтобы облегчить обновление до React 19, мы опубликовали релиз `react@18.3`, который идентичен 18.2, но добавляет предупреждения об устаревших API и других изменениях, необходимых для React 19.
...
#### Новый JSX Transform теперь обязателен {/*new-jsx-transform-is-now-required*/}

Мы представили [новый JSX transform](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) в 2020 году для улучшения размера бандла и использования JSX без импорта React. В React 19 мы добавляем дополнительные улучшения, такие как использование `ref` в качестве пропса и улучшения скорости JSX, которые требуют нового transform.

Если новый transform не включен, вы увидите это предупреждение:

<ConsoleBlockMulti>

<ConsoleLogLine level="error">

Ваше приложение (или одна из его зависимостей) использует устаревший JSX transform. Обновитесь до современного JSX transform для повышения производительности: https://react.dev/link/new-jsx-transform
...
Мы ожидаем, что большинство приложений не пострадают, так как transform уже включен в большинстве сред. Инструкции по обновлению вручную см. в [анонсе](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html).
...
Мы также включаем codemod для наиболее распространенных замен. См. [Изменения в TypeScript](#typescript-changes) ниже.
...
Это не включает изменения TypeScript. См. [Изменения в TypeScript](#typescript-changes) ниже.
...
### Ошибки при рендеринге больше не перебрасываются {/*errors-in-render-are-not-re-thrown*/}

В предыдущих версиях React ошибки, возникающие во время рендеринга, перехватывались и перебрасывались. В режиме разработки (DEV) мы также выводили сообщение в `console.error`, что приводило к дублированию логов ошибок.

В React 19 мы [улучшили обработку ошибок](/blog/2024/04/25/react-19#error-handling), чтобы уменьшить дублирование, не перебрасывая ошибки:
...
Codemod для `propTypes` в TypeScript с помощью:
...
#### Удалено: Устаревший Context с использованием `contextTypes` и `getChildContext` {/*removed-removing-legacy-context*/}

Устаревший Context был объявлен устаревшим в [октябре 2018 г. (v16.6.0)](https://legacy.reactjs.org/blog/2018/10/23/react-v-16-6.html).

Устаревший Context был доступен только в классовых компонентах с использованием API `contextTypes` и `getChildContext`, и был заменен на `contextType` из-за тонких ошибок, которые было легко пропустить. В React 19 мы удаляем устаревший Context, чтобы сделать React немного меньше и быстрее.

Если вы все еще используете устаревший Context в классовых компонентах, вам нужно будет перейти на новый API `contextType`:
...
Codemod для строковых рефов в колбэки рефов:
...
#### Удалено: фабрики шаблонного паттерна {/*removed-module-pattern-factories*/}
Фабрики шаблонного паттерна были объявлены устаревшими в [августе 2019 г. (v16.9.0)](https://legacy.reactjs.org/blog/2019/08/08/react-v16.9.0.html#deprecating-module-pattern-factories).

Этот паттерн использовался редко, и его поддержка делает React немного больше и медленнее, чем необходимо. В React 19 мы удаляем поддержку фабрик шаблонного паттерна, и вам нужно будет перейти на обычные функции:
...
Использование `createFactory` было распространено до широкой поддержки JSX, но сегодня используется редко и может быть заменено JSX. В React 19 мы удаляем `createFactory`, и вам нужно будет перейти на JSX:
...
##### Пожалуйста, пересмотрите использование shallow rendering {/*please-reconsider-shallow-rendering*/}

Shallow rendering зависит от внутренних механизмов React и может блокировать вас от будущих обновлений. Мы рекомендуем перенести ваши тесты на [@testing-library/react](https://testing-library.com/docs/react-testing-library/intro/) или [@testing-library/react-native](https://testing-library.com/docs/react-native-testing-library/intro).
...
Все остальные функции `test-utils` были удалены. Эти утилиты использовались редко, и позволяли слишком легко зависеть от низкоуровневых деталей реализации ваших компонентов и React. В React 19 эти функции будут вызывать ошибку при вызове, а их экспорты будут удалены в будущей версии.

См. [страницу предупреждений](https://react.dev/warnings/react-dom-test-utils) для альтернатив.

<Note>

Codemod для `ReactDOMTestUtils.act` в `React.act`:
...
Codemod для `ReactDOM.render` в `ReactDOMClient.createRoot`:
...
Codemod для `ReactDOM.hydrate` в `ReactDOMClient.hydrateRoot`:
...
Codemod для `unmountComponentAtNode` в `root.unmount`:
...
Мы удаляем `findDOMNode`, поскольку это был устаревший обходной путь, который медленно выполнялся, был хрупким к рефакторингу, возвращал только первого потомка и нарушал уровни абстракции (подробнее см. [здесь](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Вы можете заменить `ReactDOM.findDOMNode` на [DOM-рефы](/learn/manipulating-the-dom-with-refs):
...
Мы объявляем `react-test-renderer` устаревшим, поскольку он реализует собственную среду рендеринга, которая не соответствует среде, используемой пользователями, способствует тестированию деталей реализации и полагается на интроспекцию внутренних механизмов React.
...
Как и все поведения Strict Mode, эти функции предназначены для проактивного выявления ошибок в ваших компонентах во время разработки, чтобы вы могли исправить их до того, как они будут выпущены в продакшен. Например, во время разработки Strict Mode будет дважды вызывать функции колбэков рефов при первоначальном монтировании, чтобы имитировать то, что происходит, когда смонтированный компонент заменяется запасным элементом Suspense.

### Улучшения Suspense {/*improvements-to-suspense*/}

В React 19, когда компонент приостанавливается (suspends), React немедленно применит запасной элемент ближайшего границы Suspense, не дожидаясь рендеринга всего дерева сиблингов. После применения запасного элемента React запланирует еще один рендер для приостановленных сиблингов, чтобы "предварительно разогреть" ленивые запросы в остальной части дерева:
...
Ранее, когда компонент приостанавливался, рендерились приостановленные сиблинги, а затем применялся запасной элемент.
...
В React 19, когда компонент приостанавливается, применяется запасной элемент, а затем рендерятся приостановленные сиблинги.

</Diagram>

Это изменение означает, что запасные элементы Suspense отображаются быстрее, одновременно разогревая ленивые запросы в приостановленном дереве.
...
В будущем мы будем более агрессивно блокировать доступ к внутренним механизмам React, чтобы препятствовать их использованию и гарантировать, что пользователи не будут заблокированы от обновлений.

## Изменения в TypeScript {/*typescript-changes*/}

### Удалены устаревшие типы TypeScript {/*removed-deprecated-typescript-types*/}

Мы очистили типы TypeScript в соответствии с удалёнными API в React 19. Некоторые из удалённых типов были перемещены в более релевантные пакеты, а другие больше не нужны для описания поведения React.
...
// Нельзя присвоить значение свойству 'current', так как оно доступно только для чтения
ref.current = 1;
```
...
Ознакомьтесь с [[RFC] Сделать все рефы изменяемыми](https://github.com/DefinitelyTyped/DefinitelyTyped/pull/64772) для предварительного обсуждения этого изменения.
...
`props` React-элементов теперь по умолчанию имеют тип `unknown` вместо `any`, если элемент типизирован как `ReactElement`. Это не повлияет на вас, если вы передаете аргумент типа в `ReactElement`:
...
Вам это понадобится только в том случае, если у вас много устаревшего кода, полагающегося на некорректный доступ к пропсам элемента. Интроспекция элементов существует только как крайняя мера, и вам следует явно указывать, что ваш доступ к пропсам некорректен, используя явный `any`.
...
Точный спецификатор модуля зависит от JSX-среды выполнения, которую вы указали в `compilerOptions` вашего `tsconfig.json`:
...
Однако это потребовало критического изменения, при котором `useReducer` не принимает полный тип редьюсера в качестве параметра типа, а вместо этого либо не требует никакого (и полагается на контекстное типирование), либо требует тип состояния и действия.
...
+ useReducer(reducer)
```
Это может не работать в крайних случаях, когда вы можете явно типизировать состояние и действие, передавая `Action` в виде кортежа:
```diff
- useReducer<React.Reducer<State, Action>>(reducer)
...
```
Это также то, что вам придется сделать, если вы переместите редьюсер за пределы вызова `useReducer`:
...
- **react-dom**: Удален `errorInfo.digest` из `onRecoverableError` [#28222](https://github.com/facebook/react/pull/28222)
- **react-dom**: Удален `unstable_flushControlled` [#26397](https://github.com/facebook/react/pull/26397)
- **react-dom**: Удален `unstable_createEventHandle` [#28271](https://github.com/facebook/react/pull/28271)
- **react-dom**: Удален `unstable_renderSubtreeIntoContainer` [#28271](https://github.com/facebook/react/pull/28271)
- **react-dom**: Удален `unstable_runWithPriority` [#28271](https://github.com/facebook/react/pull/28271)
...
- **react**: Пакетные, синхронные, стандартные и непрерывные дорожки [#25700](https://github.com/facebook/react/pull/25700)
...
- **react-dom**: Предупреждение о `layout effect` во время SSR удалено [#26395](https://github.com/facebook/react/pull/26395)
- **react-dom**: Предупреждение и установка пустой строки для src/href (кроме тегов якоря) [#28124](https://github.com/facebook/react/pull/28124)
